...
// APIClient manages communication with the Infrahub-API API v1.0
...
type APIClient struct {
	...
	AssigningMemberRoleAPI *AssigningMemberRoleAPIService
	...
	ClusterEventsAPI *ClusterEventsAPIService
	...
	FirewallAttachmentAPI *FirewallAttachmentAPIService

	FirewallsAPI *FirewallsAPIService

	FlavorAPI *FlavorAPIService
	...
	PricebookAPI *PricebookAPIService
	...
	SnapshotEventsAPI *SnapshotEventsAPIService
	...
	VirtualMachineAPI *VirtualMachineAPIService

	VirtualMachineEventsAPI *VirtualMachineEventsAPIService

	VncUrlAPI *VncUrlAPIService
...
func NewAPIClient(cfg *Configuration) *APIClient {
	...
	c.PricebookAPI = (*PricebookAPIService)(&c.common)
	...
	c.VirtualMachineEventsAPI = (*VirtualMachineEventsAPIService)(&c.common)
...
func CacheExpires(r *http.Response) CustomTime {
	// Figure out when the cache expires.
	var expires CustomTime
	now, err := ParseCustomTime(time.RFC1123, r.Header.Get("date"))
	if err != nil {
	...
	if maxAge, ok := respCacheControl["max-age"]; ok {
		...
		if err != nil {
			...
			expires = NewCustomTime(NewCustomTime(time.Now()).Add(lifetime))
		}
	} else {
		expiresHeader := r.Header.Get("Expires")
		if expiresHeader != "" {
			expires, err = ParseCustomTime(time.RFC1123, expiresHeader)
			if err != nil {
				expires = now
			}
		}
