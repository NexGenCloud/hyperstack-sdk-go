// Package alive provides primitives to interact with the openapi HTTP API.
//
...
func NewGetAliveRequest(server string) (*http.Request, error) {
	...
	operationPath := fmt.Sprintf("/billing/alive")
	if operationPath[0] == '/' {
...
func NewGetDocRequest(server string) (*http.Request, error) {
	...
	operationPath := fmt.Sprintf("/billing/alive/doc")
	if operationPath[0] == '/' {
		operationPath = "." + operationPath
	}
