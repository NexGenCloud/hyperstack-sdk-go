...
/*
GetEnvironment Retrieve environment
...
*/
func (a *EnvironmentAPIService) GetEnvironment(ctx context.Context, id int32) ApiGetEnvironmentRequest {
	return ApiGetEnvironmentRequest{
		ApiService: a,
		ctx:        ctx,
		id:         id,
...
type ApiGetEnvironmentVMQuotaRequest struct {
	ctx        context.Context
	ApiService *EnvironmentAPIService
	id         int32
}

func (r ApiGetEnvironmentVMQuotaRequest) Execute() (*VMQuota, *http.Response, error) {
	return r.ApiService.GetEnvironmentVMQuotaExecute(r)
}

/*
GetEnvironmentVMQuota Get environment VM quota
...
Retrieves VM quota information for a specific environment, including current VM count, maximum VMs allowed, available VMs, and percentage used in an environment before reaching the subnet IP limit.
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param id
	@return ApiGetEnvironmentVMQuotaRequest
*/
func (a *EnvironmentAPIService) GetEnvironmentVMQuota(ctx context.Context, id int32) ApiGetEnvironmentVMQuotaRequest {
	return ApiGetEnvironmentVMQuotaRequest{
		ApiService: a,
		ctx:        ctx,
		id:         id,
...
//
//	@return VMQuota
func (a *EnvironmentAPIService) GetEnvironmentVMQuotaExecute(r ApiGetEnvironmentVMQuotaRequest) (*VMQuota, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *VMQuota
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "EnvironmentAPIService.GetEnvironmentVMQuota")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/core/environments/{id}/vm-quota"
	localVarPath = strings.Replace(localVarPath, "{"+"id"+"}", url.PathEscape(parameterValueToString(r.id, "id")), -1)
